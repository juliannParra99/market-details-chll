...
export default async function ItemsPage({searchParams}:{searchParams: {search:string}}){
  ...
  const {results} = await fetch(`https://api.mercadolibre.com/sites/MLA/search?q=${searchParams.search}&limit=7
      ...
      <article className="grid gap-4">
        ...
        {results.map((item)=>(
          <div key={item.id} className="flex gap-4">
            ...
            <span className="ml-auto  text-sm capitalize opacity-50">{item.seller_address ? item.seller_address.city.name.toLowerCase() : ''}</span>
          </div>
        ))}
      </article>
    </section>
